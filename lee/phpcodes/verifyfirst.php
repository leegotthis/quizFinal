...
$sql    = "SELECT * FROM users WHERE username = '" . $username . "'"; /** checks if profile exists */
$select = mysql_query($sql);
if (mysql_num_rows($select) > 0) {
    $sql    = "SELECT * FROM users WHERE username = '" . $username . "' AND password = '" . $pass . "'"; /** checks if Password matches with the username given */
    $select = mysql_query($sql);
    if (mysql_num_rows($select) > 0) {
        $verifycode = "v1";
        /** v1 = Profile Verified */
